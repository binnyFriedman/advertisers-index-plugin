						...
						<div class="slider--container">
							<div class="slider--view">
								<ul>
									...
									$more_advertisers = get_posts( array(
									...
									$count = 0;
									if(count($more_advertisers)) {
										for ($i=0;$i<count($more_advertisers);wp_is_mobile()?$i++:$i+=4){
											// $count++;
											$post = $more_advertisers[$i];
										...
										<li class="slider--view__slides <? echo $count==1?"is-active":"";?>">
											<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>">
												<?php if (has_post_thumbnail()) { ?>
													<div class="bg-cover bg-center b g-no-repeat rounded-full mb-6" style="width: 164px;height: 164px; background-image: url('<?php the_post_thumbnail_url(); ?>')" ></div>
												<?php } else { ?>
													<?php the_title(); ?>
												<?php } ?>
